...
export default function Records({ onNavigate }) {
  const { darkMode, theme, activeCat, user } = useApp()
  const [showAddRecord, setShowAddRecord] = useState(false)
  const [records, setRecords] = useState([])
  const [newRecord, setNewRecord] = useState({ date: '', type: 'checkup', clinic: '', notes: '' })
  ...
  const handleAddRecord = async () => {
    if (newRecord.date && newRecord.type && activeCat?.id) {
      const recordData = {
      ...
      setNewRecord({ date: '', type: 'checkup', clinic: '', notes: '' })
  ...
  const stats = [
    { label: 'Total Visits', value: records.length, color: '#4f46e5' },
    { label: 'This Year', value: records.filter(r => new Date(r.date).getFullYear() === 2026).length, color: '#7c3aed' },
    { label: 'Documents', value: records.filter(r => r.hasFiles).length, color: '#ec4899' },
  ]
  ...
  return (
    <div style={{ minHeight: '100vh', background: theme.bg }}>
      <header style={{ padding: '16px 24px', display: 'flex', alignItems: 'center', gap: '14px', borderBottom: `1px solid ${theme.border}`, background: theme.card }}>
        ...
        <div style={{ flex: 1 }}><h1 style={{ fontSize: '20px', fontWeight: '700', color: theme.text, margin: 0 }}>Medical Records</h1><p style={{ fontSize: '12px', color: theme.textMuted, margin: '2px 0 0' }}>{activeCat?.name}'s health history</p></div>
        <button onClick={() => setShowAddRecord(true)} style={{ height: '38px', padding: '0 14px', borderRadius: '10px', border: 'none', background: 'linear-gradient(135deg, #4f46e5, #7c3aed)', color: 'white', fontSize: '12px', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}><Plus /> Add Record</button>
      </header>
